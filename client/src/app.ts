...
var params: any = {  // Object for parameters sent to the Watson Conversation service
    input: '',
    context: '',
};
var watson = 'Bot';
var user = '';
var context: any;  // Very important. Holds all the data for the current point of the chat.
 ...
 */
function newEvent(e: any) {
    // Only check for a return/enter press - Event 13
    if (e.which === 13 || e.keyCode === 13) {

        var userInput: any = document.getElementById('chatMessage');
...
function userMessage(message: string) {
    ...
    // User defined text to be sent to service
    const input = {
        text: message
    }
    ...
    if (context) {
        ...
        params.context.username = "Innovatec";
    }

    var xhr = new XMLHttpRequest();
    var uri = '/api/conversation/message';
    ...
    xhr.onload = function () {
        ...
        if (xhr.status === 200 && xhr.responseText) {
            ...
            var responseTextList = response.output.text;
            console.log(responseTextList);
            for (var i = 0; i < responseTextList.length; i++) {
                displayMessage(responseTextList[i], watson);
            }
        } else {
            console.error("Server", 'Server error for Conversation. Return status of: ', xhr.statusText);
        }
    };

    xhr.onerror = function () {
        console.error("Server", 'Network error trying to send message!');
    };
 ...
 * @return null
 */
function displayMessage(text: string, user: string) {
